...
import { Calendar, Search as SearchIcon, ExternalLink, Loader2, CheckCircle2 } from "lucide-react";
...
export default function SearchEnhanced() {
  ...
  const [searchQueries, setSearchQueries] = useState<Array<{ query: string; description: string }>>([]);
  ...
  const { data: leads, isLoading: isLoadingResults, error } = trpc.leads.search.useQuery(
    {
      state: selectedState,
      city: selectedCity,
  ...
  const { data: searchData } = trpc.leads.searchWithProgress.useQuery(
    {
      state: selectedState,
      city: selectedCity,
      startDate: startDateObj,
  ...
  const simulateSearch = async (queries: Array<{ query: string; description: string }>) => {
    setIsSearching(true);
    for (let i = 0; i < queries.length; i++) {
      setCurrentSearchIndex(i);
      setCompletedQueries(i + 1);
  ...
  const handleSearch = async () => {
    if (selectedState && selectedCity && isDateValid && !isEndDateFuture) {
      try {
        ...
        const response = await utils.leads.searchWithProgress.fetch({
          state: selectedState,
          city: selectedCity,
          startDate: startDateObj,
          endDate: endDateObj,
        });
        setSearchQueries(response.queries);
        setCompletedQueries(0);
        simulateSearch(response.queries);
      } catch (err) {
  ...
  const progressPercentage = searchQueries.length > 0 ? (completedQueries / searchQueries.length) * 100 : 0;
      ...
      <div className="max-w-6xl mx-auto">
        <div className="mb-8">
          ...
          <p className="text-lg text-slate-600">
            Encontre potenciais clientes em comentários de imobiliárias no Instagram
          </p>
        </div>
        ...
        {isSearching && searchQueries.length > 0 && (
          <Card className="mb-8 shadow-lg border-blue-200 bg-blue-50">
            <CardHeader>
              <CardTitle className="text-blue-900">Progresso da Busca</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <div className="text-center">
                ...
                {currentQuery && (
                  <div className="bg-white p-4 rounded-lg border border-blue-200">
                    ...
                    <p className="text-base font-medium text-blue-700">{currentQuery.description}</p>
                    <p className="text-xs text-slate-500 mt-2 break-all">{currentQuery.query}</p>
                  </div>
                )}
              </div>
              ...
              <div className="space-y-2">
                <p className="text-sm font-medium text-slate-700">
                  Buscas Concluídas: {completedQueries} de {searchQueries.length}
                </p>
        ...
        {!isSearching && leads && leads.length > 0 && (
          <div>
        ...
        {!isSearching && leads && leads.length === 0 && completedQueries > 0 && (
          <Card className="border-slate-200 bg-slate-50">
            <CardContent className="pt-6">
              <p className="text-slate-600 text-center">
                Nenhum lead encontrado para os critérios selecionados. Tente ajustar os filtros.
              </p>
            </CardContent>
          </Card>
        )}

        {!isSearching && completedQueries === 0 && (
          <Card className="border-slate-200 bg-slate-50">
            <CardContent className="pt-6">
              <p className="text-slate-600 text-center">
                Preencha os filtros acima e clique em "Buscar Leads" para começar a procurar.
              </p>
            </CardContent>
          </Card>
        )}
